...
import * as lean from './checksum-bridge.mjs';
const adapters = {
  1: { ...old, create: (_, cells) => old.create(128, cells), deserialize: (_, cells, bytes) => old.deserialize(128, cells, bytes), input: keys => old.input(keys, 16), decode: (a,b) => old.decode(a,b,16) },
  2: { ...lean, create: (_, cells) => lean.create(32, cells), deserialize: (_, cells, bytes) => lean.deserialize(32, cells, bytes), input: keys => lean.input(keys), decode: (a,b) => lean.decode(a,b) },
  3: { ...lean, create: (_, cells) => lean.create(96, cells), deserialize: (_, cells, bytes) => lean.deserialize(96, cells, bytes), input: keys => lean.input(keys), decode: (a,b) => lean.decode(a,b) }
};
let lib;
const labels = { 8: 'legacy128/check32', 16: 'lean128/check32', 24: 'lean128/check96' };

const n = Number(process.env.N ?? 10000), diff = Number(process.env.DIFF ?? 100);
const cells = Number(process.env.CELLS ?? 512);
const trials = Number(process.env.TRIALS ?? 30), warmups = Number(process.env.WARMUPS ?? 5);
...
const half = diff / 2, widths = [1, 2, 3], now = () => performance.now();
...
const report = { environment: { node: process.version, cpu: cpus()[0]?.model, platform: process.platform, arch: process.arch },
  config: { n, diff, cells, hashCount: 4, keyBits: 128, trials, warmups,
    dataset: 'identical first 16 bytes of SHA-1 digests for all three variants; key generation excluded',
  ...
  build: JSON.parse(await readFile(new URL('./build/checksum/build-info.json', import.meta.url))), summary, runs };
await writeFile(process.env.OUTPUT ?? new URL('./results-checksum.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
