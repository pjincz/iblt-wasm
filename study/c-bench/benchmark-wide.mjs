...
import * as lib from './wide-bridge.mjs';
...
const half = diff / 2, widths = [8, 16, 20], now = () => performance.now();
...
try {
  for (let r = -warmups; r < trials; ++r) {
    ...
    for (let j = 0; j < states.length; ++j) {
      ...
      const inputs = [oldA, oldB, onlyA, onlyB].map(keys => lib.input(keys.map(x => x.subarray(0, width)), width));
      try {
...
} finally { states.forEach(({ a, b }) => { lib.destroy(a); lib.destroy(b); }); }
...
for (const [scenario, cases] of Object.entries(runs)) {
  summary[scenario] = Object.entries(cases).map(([bits, rows]) => {
    const out = { bits: Number(bits), success: `${rows.filter(x => x.success).length}/${rows.length}`, wrongResults: rows.filter(x => x.wrongResult).length };
...
const report = { environment: { node: process.version, cpu: cpus()[0]?.model, platform: process.platform, arch: process.arch },
  config: { n, diff, cells, hashCount: 4, checksumBits: 32, trials, warmups,
    dataset: 'same SHA-1 digests per round; 64-bit first 8 bytes, 128-bit first 16 bytes, 160-bit all 20 bytes',
  ...
  build: JSON.parse(await readFile(new URL('./build/wide/build-info.json', import.meta.url))), summary, runs };
await writeFile(process.env.OUTPUT ?? new URL('./results-wide.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
