import createModule from './iblt-dynamic.mjs';
...
const finalizer = new FinalizationRegistry(state => {
  if (state.ptr) {
    wasm._dynamic_destroy(state.ptr);
    state.ptr = 0;
  }
});
...
export function clone(table) {
  ...
  return wrap(wasm._dynamic_clone(ptr), keyBytes, checkBytes, cells);
}
function change(table, keys, remove) {
  ...
  try {
    ...
    if (!wasm._dynamic_update(tablePtr, ptr, keys.length, Number(remove))) {
...
export function serialize(t) {
  const tablePtr=config(t).ptr;const length=wasm._dynamic_wire_size(tablePtr),ptr=wasm._malloc(length);
  try {wasm._dynamic_serialize(tablePtr,ptr);return wasm.HEAPU8.slice(ptr,ptr+length);}finally{wasm._free(ptr);}
}
export function deserialize(keyBytes,checkBytes,cells,bytes) {
  ...
  try{wasm.HEAPU8.set(bytes,ptr);wasm._dynamic_deserialize(config(t).ptr,ptr);return t;}catch(e){destroy(t);throw e;}finally{wasm._free(ptr);}
}
export function decode(a,b) {
  ...
  const success=Boolean(wasm._dynamic_decode(ca.ptr,cb.ptr));
  const count=wasm._dynamic_result_count(ca.ptr),keys=wasm._dynamic_result_keys(ca.ptr),sides=wasm._dynamic_result_sides(ca.ptr);
