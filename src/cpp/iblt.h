...
// Inline before lowering the array return to WASM's memory-based ABI.
#if defined(__GNUC__) || defined(__clang__)
__attribute__((always_inline))
#endif
inline std::array<uint32_t, 4> hash128(const uint8_t *key, unsigned bytes, uint32_t seed) {
    ...
    // Numeric word order is independent of host byte order: low/high h1, low/high h2.
    return {uint32_t(h1),uint32_t(h1 >> 32),uint32_t(h2),uint32_t(h2 >> 32)};
}
...
public:
    ...
    auto checksum(const uint8_t *key) const {
        return hash128(key,keyBytes_,11);
    }
    auto indexes(const uint8_t *key) const {
        std::array<unsigned,4> positions;
        const auto words = hash128(key,keyBytes_,0);
