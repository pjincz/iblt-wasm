...
const server_table = iblt.create(16, 12, 512); // key length, checksum length (bytes), cells
...
if (result.success) {
  ...
  // Discard partial results and rebuild both tables with more cells.
  console.log('Could not decode the differences; retry with more cells.');
}
```
...
- `iblt.create(keyLength, checksumLength, cells)` → Table

  Create an empty table. All three arguments are required. Lengths are in bytes.

- `iblt.deserialize(keyLength, checksumLength, cells, payload)` → Table
...
At the next sync, just call `table.serialize()` and compare against the other side's maintained table. Adding or removing each key touches four cells, regardless of the total record count, so you do not need to scan the full list on every sync. Decoding leaves both tables unchanged, ready for future updates.
...
Choose the **smallest multiple of 4 bytes that fits every key your application needs**. For example, a 64-bit ID needs 8 bytes, a 128-bit ID needs 16, and a SHA-1 digest needs 20. Shorter keys save space in every cell and reduce transfer size. For strings, measure UTF-8 byte length, not JavaScript string length. The example uses 16 bytes, but 8 would fit all three sample names.
...
The checksum helps the decoder check whether a cell contains a single recoverable key. It is separate from the key and does not carry application data. Supported lengths are **4, 8, 12 and 16 bytes** (32, 64, 96 and 128 bits). All lengths use the corresponding prefix of MurmurHash3_x64_128 (seed 11), encoded little-endian. Cell selection uses another MurmurHash3_x64_128 result (seed 0), split into four 32-bit words, one for each disjoint subtable.

Longer checksums reduce accidental false matches at the cost of more bytes per cell. **12 bytes is a reasonable starting point** and is used in our examples; 4 saves the most space but provides less protection, while 8 and 16 offer intermediate and stronger protection respectively. Increasing this value does not fix insufficient cell capacity or collisions in your application's own key encoding. Checksums use noncryptographic MurmurHash3, so they are not authentication and do not guarantee correct decoding.
...
```text
bytes = (4 + key_length + checksum_length) * cells
```

The `4` is the signed 32-bit count stored in each cell. For the example:

```text
(4 + 16 + 12) * 512 = 16,384 bytes = 16 KiB
```

This excludes transport headers, retries, additional table transfers and any actual records exchanged after finding the differences. The payload does not include the configuration; agree on it separately. Total cell storage is limited to `INT32_MAX` bytes.
